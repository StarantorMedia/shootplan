...
const S = {
  root: { fontFamily: "'DM Sans','Segoe UI',sans-serif", background: "#0A0A0F", color: "#E8E8F0", minHeight: "100vh" },
  // Sidebar
  sidebar: (open) => ({ width: 240, minHeight: "100vh", background: "#111118", borderRight: "1px solid #1E1E2E", display: "flex", flexDirection: "column", position: "fixed", top: 0, left: 0, bottom: 0, zIndex: 200, overflowY: "auto", transform: open ? "translateX(0)" : "translateX(-100%)", transition: "transform 0.25s ease" }),
  sidebarDesktop: { width: 240, minHeight: "100vh", background: "#111118", borderRight: "1px solid #1E1E2E", display: "flex", flexDirection: "column", position: "fixed", top: 0, left: 0, bottom: 0, zIndex: 200, overflowY: "auto" },
  overlay: { position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", zIndex: 150, backdropFilter: "blur(2px)" },
  logo: { padding: "24px 20px 18px", borderBottom: "1px solid #1E1E2E" },
  logoMark: { display: "flex", alignItems: "center", gap: 10 },
  logoIcon: { width: 32, height: 32, background: "linear-gradient(135deg,#6366F1,#8B5CF6)", borderRadius: 8, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 16, flexShrink: 0 },
  logoText: { fontSize: 15, fontWeight: 700, letterSpacing: "-0.3px", color: "#F0F0FA" },
  logoSub: { fontSize: 10, color: "#555570", letterSpacing: "0.8px", textTransform: "uppercase", marginTop: 1 },
  nav: { padding: "12px 10px", flex: 1, display: "flex", flexDirection: "column", gap: 2 },
  navSection: { fontSize: 10, fontWeight: 600, color: "#444460", letterSpacing: "0.8px", textTransform: "uppercase", padding: "10px 10px 5px" },
  navItem: (a) => ({ display: "flex", alignItems: "center", gap: 10, padding: "9px 12px", borderRadius: 8, cursor: "pointer", background: a ? "rgba(99,102,241,0.15)" : "transparent", color: a ? "#818CF8" : "#888899", fontSize: 14, fontWeight: a ? 600 : 400, border: a ? "1px solid rgba(99,102,241,0.2)" : "1px solid transparent", transition: "all 0.15s" }),
  navIcon: { fontSize: 15, width: 20, textAlign: "center" },
  sidebarUser: { padding: "14px", borderTop: "1px solid #1E1E2E", display: "flex", alignItems: "center", gap: 10 },
  avatar: (sz = 32) => ({ width: sz, height: sz, borderRadius: "50%", background: "linear-gradient(135deg,#6366F1,#8B5CF6)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: Math.max(sz * 0.38, 11), fontWeight: 700, color: "white", flexShrink: 0 }),
  // Main
  topbar: { display: "flex", alignItems: "center", gap: 12, padding: "14px 16px", background: "#111118", borderBottom: "1px solid #1E1E2E", position: "sticky", top: 0, zIndex: 100 },
  main: { flex: 1, padding: "24px 16px", maxWidth: "100%", boxSizing: "border-box" },
  mainDesktop: { marginLeft: 240, flex: 1, padding: "32px", maxWidth: "calc(100vw - 240px)", boxSizing: "border-box" },
  // Common
  pageHeader: { marginBottom: 24, display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 12 },
  pageTitle: { fontSize: 22, fontWeight: 700, letterSpacing: "-0.5px", color: "#F0F0FA", marginBottom: 3 },
  pageSub: { fontSize: 13, color: "#555570" },
  btn: (v = "primary") => ({ display: "inline-flex", alignItems: "center", gap: 6, padding: v === "ghost" ? "7px 12px" : "9px 16px", borderRadius: 8, border: v === "outline" ? "1px solid #2A2A3E" : "none", background: v === "primary" ? "linear-gradient(135deg,#6366F1,#7C3AED)" : v === "danger" ? "rgba(239,68,68,0.15)" : v === "outline" ? "transparent" : "rgba(255,255,255,0.04)", color: v === "primary" ? "white" : v === "danger" ? "#EF4444" : v === "outline" ? "#888899" : "#818CF8", fontSize: 13, fontWeight: 600, cursor: "pointer", whiteSpace: "nowrap", flexShrink: 0 }),
  card: { background: "#111118", border: "1px solid #1E1E2E", borderRadius: 12, padding: "16px" },
  cardHover: { background: "#111118", border: "1px solid #1E1E2E", borderRadius: 12, padding: "16px", cursor: "pointer", transition: "border-color 0.15s" },
  ...
  statCard: { background: "#111118", border: "1px solid #1E1E2E", borderRadius: 12, padding: "18px 20px" },
  statValue: { fontSize: 28, fontWeight: 700, letterSpacing: "-1px", color: "#F0F0FA" },
  statLabel: { fontSize: 11, color: "#555570", marginTop: 3, letterSpacing: "0.3px", textTransform: "uppercase" },
  badge: (s) => { const c = STATUS_CONFIG[s] || { color: "#888", bg: "rgba(128,128,128,0.1)", dot: "#888" }; return { display: "inline-flex", alignItems: "center", gap: 4, padding: "3px 9px", borderRadius: 20, fontSize: 11, fontWeight: 600, color: c.color, background: c.bg }; },
  attendBadge: (s) => { const c = ATTEND_CONFIG[s] || ATTEND_CONFIG.open; return { display: "inline-flex", alignItems: "center", gap: 4, padding: "3px 9px", borderRadius: 20, fontSize: 11, fontWeight: 600, color: c.color, background: c.bg }; },
  roleBadge: (r) => { const c = ROLE_CONFIG[r] || ROLE_CONFIG.crew; return { display: "inline-flex", alignItems: "center", gap: 4, padding: "2px 8px", borderRadius: 4, fontSize: 11, fontWeight: 600, color: c.color, background: c.color + "22" }; },
  ...
  label: { fontSize: 11, fontWeight: 600, color: "#555570", marginBottom: 5, display: "block", letterSpacing: "0.4px", textTransform: "uppercase" },
  modal: { position: "fixed", inset: 0, background: "rgba(0,0,0,0.75)", backdropFilter: "blur(4px)", display: "flex", alignItems: "flex-end", justifyContent: "center", zIndex: 1000, padding: 0 },
  modalBox: { background: "#111118", border: "1px solid #2A2A3E", borderRadius: "16px 16px 0 0", padding: "24px 20px", width: "100%", maxWidth: 600, maxHeight: "90vh", overflowY: "auto" },
  ...
  toggle: (a) => ({ padding: "7px 14px", borderRadius: 7, border: "none", background: a ? "#1E1E2E" : "transparent", color: a ? "#E8E8F0" : "#555570", fontSize: 13, fontWeight: a ? 600 : 400, cursor: "pointer" }),
  err: { fontSize: 12, color: "#EF4444", padding: "10px 12px", background: "rgba(239,68,68,0.08)", borderRadius: 6, marginBottom: 12 },
  driveBtn: { display: "inline-flex", alignItems: "center", gap: 8, padding: "8px 14px", borderRadius: 8, border: "1px solid #2A2A3E", background: "rgba(99,102,241,0.08)", color: "#818CF8", fontSize: 13, fontWeight: 600, cursor: "pointer", textDecoration: "none" },
  divider: { height: 1, background: "#1E1E2E", margin: "16px 0" },
  hamburger: { background: "none", border: "none", color: "#E8E8F0", cursor: "pointer", fontSize: 22, padding: "4px", display: "flex", alignItems: "center" },
};
    ...
    <div style={{ minHeight: "100vh", background: "#0A0A0F", display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}>
      <div style={{ width: "100%", maxWidth: 400 }}>
        <div style={{ textAlign: "center", marginBottom: 32 }}>
          <div style={{ width: 52, height: 52, background: "linear-gradient(135deg,#6366F1,#8B5CF6)", borderRadius: 14, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 24, margin: "0 auto 14px" }}>🎬</div>
          <div style={{ fontSize: 24, fontWeight: 800, letterSpacing: "-0.8px", color: "#F0F0FA" }}>ShootPlan</div>
          <div style={{ fontSize: 13, color: "#555570", marginTop: 5 }}>Professionelle Videoproduktions-Planung</div>
        </div>
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0A0A0F", display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}>
      <div style={{ width: "100%", maxWidth: 380 }}>
        <div style={{ textAlign: "center", marginBottom: 28 }}><div style={{ fontSize: 22, fontWeight: 800, color: "#F0F0FA" }}>Neues Passwort setzen</div></div>
        <div style={{ ...S.card, padding: 24 }}>
      ...
      <div style={S.root}>
        ...
        <div style={{ minHeight: "100vh", display: "flex", flexDirection: "column" }}>
          <div style={S.topbar}>
            ...
            <div style={{ flex: 1 }}><div style={{ fontSize: 15, fontWeight: 700, color: "#F0F0FA" }}>ShootPlan</div></div>
...
// SHOOT DETAIL
// ============================================================
function ShootDetail({ shoot, setShoot, participants, setParticipants, shotlist, setShotlist, schedule, setSchedule, users, clients, user, setPage, onDelete }) {
  ...
  const tabs = [["overview","Übersicht"],["documents","Dokumente"],["shotlist","Shotlist"],["schedule","Tagesplan"],["crew","Crew"]];
  ...
  return (
    <div>
      ...
      {tab === "schedule" && (
        <div>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
            <div style={{ fontSize: 15, fontWeight: 600, color: "#E8E8F0" }}>Tagesplan ({sched.length})</div>
            {user.is_admin && <button style={S.btn("primary")} onClick={addSched}>＋</button>}
          </div>
          <div style={{ borderLeft: "2px solid #1E1E2E", paddingLeft: 16, marginLeft: 8 }}>
            {sched.map(entry => (
              <div key={entry.id} style={{ position: "relative", paddingBottom: 16 }}>
                <div style={{ position: "absolute", left: -23, top: 4, width: 8, height: 8, borderRadius: "50%", background: "#6366F1", border: "2px solid #0A0A0F" }}/>
                {user.is_admin ? (
                  <div style={{ ...S.card, padding: 12 }}>
                  ...
                  <div style={{ display: "flex", gap: 10, alignItems: "baseline", paddingBottom: 4 }}>
                    <span style={{ fontSize: 13, fontWeight: 800, color: "#818CF8", minWidth: 40 }}>{entry.time}</span>
                    <div><div style={{ fontSize: 13, fontWeight: 600, color: "#E8E8F0" }}>{entry.title}</div>{entry.description && <div style={{ fontSize: 12, color: "#555570" }}>{entry.description}</div>}</div>
                  </div>
                )}
              </div>
            ))}
            {sched.length===0 && <div style={{ color:"#444460", fontSize:13, padding:16 }}>Kein Tagesplan erstellt</div>}
          </div>
        </div>
      )}
    </div>
...
export default function App() {
  ...
  const loadAll = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const [u, s, p, sl, sc, cl] = await Promise.all([db.select("users"), db.select("shoots"), db.select("shoot_participants"), db.select("shotlist"), db.select("schedule"), db.select("clients")]);
      setUsers(u); setShoots(s); setParticipants(p); setShotlist(sl); setSchedule(sc); setClients(cl);
  ...
  const handleLogout = () => { db.clearToken(); setUser(null); setShoots([]); setParticipants([]); setShotlist([]); setSchedule([]); setUsers([]); setClients([]); setPage("dashboard"); };
  ...
  const content = loading ? (
    <div style={{ textAlign: "center", padding: 60, color: "#555570" }}>Lade...</div>
  ) : (
    <>
      ...
      {page === "shoot-detail" && selectedShoot && <ShootDetail shoot={selectedShoot} setShoot={setSelectedShoot} participants={participants} setParticipants={setParticipants} shotlist={shotlist} setShotlist={setShotlist} schedule={schedule} setSchedule={setSchedule} users={users} clients={clients} user={user} setPage={setPage} onDelete={handleDeleteShoot} />}
    </>
  );

  return (
    <>
      <style>{`* { box-sizing: border-box; } body { margin: 0; background: #0A0A0F; } input:focus,textarea:focus,select:focus { border-color: #6366F1 !important; box-shadow: 0 0 0 3px rgba(99,102,241,0.12); } ::-webkit-scrollbar { width: 5px; } ::-webkit-scrollbar-track { background: #0A0A0F; } ::-webkit-scrollbar-thumb { background: #2A2A3E; border-radius: 3px; } button:hover { opacity: 0.85; } a:hover { opacity: 0.8; }`}</style>
