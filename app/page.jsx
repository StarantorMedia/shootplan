...
const db = {
  ...
  async select(table, filter = "") {
    ...
    if (!r.ok) throw new Error(d?.message || "Fehler beim Laden");
    return d;
  },
  async insert(table, body) {
    const r = await fetch(`${SUPABASE_URL}/rest/v1/${table}`, { method: "POST", headers: getHeaders(), body: JSON.stringify(body) });
    const d = await r.json();
    if (!r.ok) throw new Error(d?.message || "Fehler beim Speichern");
  ...
  async update(table, body, filter) {
    ...
    const d = await r.json();
    if (!r.ok) throw new Error(d?.message || "Fehler beim Aktualisieren");
    return Array.isArray(d) ? d[0] : d;
  },
  async remove(table, filter) {
    const r = await fetch(`${SUPABASE_URL}/rest/v1/${table}?${filter}`, { method: "DELETE", headers: getHeaders() });
    if (!r.ok) { const d = await r.json(); throw new Error(d?.message || "Fehler beim Löschen"); }
  },
  async signIn(email, password) {
    ...
    const d = await r.json();
    if (!r.ok) throw new Error(d?.error_description || d?.message || "Login fehlgeschlagen");
    return d;
  },
};
...
function LoginPage({ onLogin }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!profiles.length) throw new Error("Kein Benutzerprofil gefunden. Bitte Admin kontaktieren.");
...
function ChangePasswordPage({ user, onDone }) {
  ...
  const handleSave = async () => {
    ...
    try { await db.update("users", { must_change_password: false }, `id=eq.${user.id}`); onDone(); }
    catch (e) { setError(e.message); }
  };
  return (
    <div style={{ minHeight: "100vh", background: "#0A0A0F", display: "flex", alignItems: "center", justifyContent: "center" }}>
      <div style={{ width: 380 }}>
        <div style={{ textAlign: "center", marginBottom: 32 }}><div style={{ fontSize: 24, fontWeight: 800, color: "#F0F0FA" }}>Neues Passwort</div><div style={{ fontSize: 13, color: "#555570", marginTop: 6 }}>Hallo {user.name}</div></div>
        <div style={{ ...S.card, padding: 28 }}>
  ...
  return (
    <div>
      ...
      <div style={{ ...S.grid3, marginBottom: 28 }}>
        ...
        <div style={S.statCard}><div style={S.statValue}>{visible.filter(s => { const d = new Date(s.date); return d.getMonth() === now.getMonth() && d.getFullYear() === now.getFullYear(); }).length}</div><div style={S.statLabel}>DIESEN MONAT</div></div>
      </div>
        ...
        {upcoming.slice(0, 5).map(shoot => {
          ...
          return (
            <div key={shoot.id} style={{ ...S.cardHover, display: "flex", alignItems: "center", gap: 16 }} onClick={() => { setSelectedShoot(shoot); setPage("shoot-detail"); }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}><div style={{ fontSize: 15, fontWeight: 600, color: "#E8E8F0", marginBottom: 3 }}>{shoot.title}</div><div style={{ fontSize: 12, color: "#555570" }}>{fmt(shoot.date)} · {shoot.start_time}–{shoot.end_time} · {shoot.location}</div></div>
...
function CalendarView({ shoots, user, setSelectedShoot, setPage }) {
  ...
  const getShootsForDay = (d) => { if (!d) return []; const ds = `${year}-${String(month+1).padStart(2,"0")}-${String(d).padStart(2,"0")}`; return shoots.filter(s => s.date === ds); };
  return (
    <div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(7,1fr)", gap: 3 }}>
          {cells.map((day, idx) => (
            <div key={idx} style={{ minHeight: 80, background: isToday(day) ? "rgba(99,102,241,0.1)" : "#0A0A0F", borderRadius: 8, padding: "6px 8px", border: isToday(day) ? "1px solid rgba(99,102,241,0.4)" : "1px solid transparent" }}>
              {day && (<><div style={{ fontSize: 13, fontWeight: isToday(day) ? 800 : 400, color: isToday(day) ? "#818CF8" : "#555570", marginBottom: 4 }}>{day}</div>{getShootsForDay(day).map(s => { const c = STATUS_CONFIG[s.status]||STATUS_CONFIG.planned; return <div key={s.id} style={{ fontSize: 10, fontWeight: 600, padding: "2px 5px", borderRadius: 4, background: c.color+"22", color: c.color, marginBottom: 2, cursor: "pointer", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }} onClick={() => { setSelectedShoot(s); setPage("shoot-detail"); }}>{s.title}</div>; })}</>)}
            </div>
          ))}
        </div>
      </div>
...
function ShootsList({ user, shoots, participants, setPage, setSelectedShoot }) {
  ...
  const myIds = participants.filter(p => p.user_id === user.id).map(p => p.shoot_id);
  const visible = user.is_admin ? shoots : shoots.filter(s => myIds.includes(s.id));
  const sorted = [...(filter === "all" ? visible : visible.filter(s => s.status === filter))].sort((a,b) => new Date(a.date)-new Date(b.date));
  return (
    <div>
      <div style={S.pageHeader}>
      ...
      {view==="list" ? (
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          {sorted.map(shoot => {
            const sp = participants.filter(p=>p.shoot_id===shoot.id); const myP = sp.find(p=>p.user_id===user.id); const sc = STATUS_CONFIG[shoot.status]||STATUS_CONFIG.planned; const sick = sp.filter(p=>p.attendance_status==="sick").length;
            return (
              <div key={shoot.id} style={{ ...S.cardHover, display: "flex", gap: 16, alignItems: "center" }} onClick={()=>{ setSelectedShoot(shoot); setPage("shoot-detail"); }}>
                <div style={{ minWidth: 52, textAlign: "center" }}><div style={{ fontSize: 20, fontWeight: 800, color: "#F0F0FA" }}>{new Date(shoot.date).getDate().toString().padStart(2,"0")}</div><div style={{ fontSize: 11, color: "#444460", textTransform: "uppercase" }}>{new Date(shoot.date).toLocaleString("de-DE",{month:"short"})}</div></div>
                <div style={{ width: 1, height: 40, background: "#1E1E2E" }} />
                <div style={{ flex: 1, minWidth: 0 }}><div style={{ fontSize: 15, fontWeight: 600, color: "#E8E8F0", marginBottom: 3 }}>{shoot.title}</div><div style={{ fontSize: 12, color: "#555570" }}>📍 {shoot.location} · {shoot.start_time}–{shoot.end_time}</div></div>
...
function ShootDetail({ shoot, setShoot, participants, setParticipants, shotlist, setShotlist, schedule, setSchedule, users, user, setPage, onDelete }) {
  ...
  const handleSave = async () => { setSaving(true); try { await db.update("shoots",{title:form.title,location:form.location,date:form.date,start_time:form.start_time,end_time:form.end_time,budget:form.budget,notes:form.notes,status:form.status},`id=eq.${shoot.id}`); setShoot({...shoot,...form}); setEditMode(false); } catch(e){alert(e.message);} setSaving(false); };
              ...
              <div style={S.card}>
                ...
                <div style={{ display:"flex", gap:6, flexWrap:"wrap" }}>
                  {Object.entries(ATTEND_CONFIG).map(([k,v])=>(
                    <button key={k} style={{ padding:"5px 12px", borderRadius:20, border:`1px solid ${myP.attendance_status===k?v.color:"#2A2A3E"}`, background:myP.attendance_status===k?v.bg:"transparent", color:myP.attendance_status===k?v.color:"#555570", fontSize:11, fontWeight:600, cursor:"pointer" }} onClick={()=>handleStatusChange(myP.id,k)}>{v.label}</button>
                  ))}
                </div>
              </div>
            )}
      ...
      {tab==="crew"&&(
        <div>
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
            {sp.map(p=>{ const u=users.find(u=>u.id===p.user_id)||{name:"Unbekannt",email:""};
              return(<div key={p.id} style={{ ...S.card, display:"flex", alignItems:"center", gap:12 }}>
        ...
        <div>
          ...
          <div style={{ borderLeft:"2px solid #1E1E2E", paddingLeft:20, marginLeft:20 }}>
            {sched.map(entry=>{
              ...
              return(<div key={entry.id} style={{ position:"relative", paddingBottom:20 }}>
                ...
                {user.is_admin?(
                  ...
                  <div style={{ paddingBottom:4, display:"flex", gap:12, alignItems:"baseline" }}>
                    ...
                    <div><div style={{ fontSize:14, fontWeight:600, color:"#E8E8F0" }}>{entry.title}</div>{entry.description&&<div style={{ fontSize:12, color:"#555570" }}>{entry.description}</div>}{assignedUser&&<div style={{ fontSize:11, color:"#444460", marginTop:2 }}>→ {assignedUser.name}</div>}</div>
                  </div>
                )}
              </div>);
            })}
...
function NewShootPage({ user, setPage, onSave }) {
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try { const shoot=await db.insert("shoots",{...form,budget:form.budget||null,created_by:user.id}); onSave(shoot); }
    catch(e){setError(e.message);}
    setSaving(false);
  };
  return (
...
function UsersPage({ users, setUsers, user: currentUser }) {
  ...
  return (
    <div>
      <div style={S.pageHeader}><div><div style={S.pageTitle}>Benutzer</div><div style={S.pageSub}>{users.length} Personen im System</div></div><button style={S.btn("primary")} onClick={()=>setShowModal(true)}>＋ Benutzer anlegen</button></div>
      <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
        {users.map(u=>(
          <div key={u.id} style={{ ...S.card, display:"flex", alignItems:"center", gap:14 }}>
            <div style={S.avatar(40)}>{u.name?.[0]}</div>
            <div style={{ flex:1 }}><div style={{ fontSize:14, fontWeight:600, color:"#E8E8F0", display:"flex", gap:8, alignItems:"center" }}>{u.name}{u.is_admin&&<span style={S.tag("#F59E0B")}>Admin</span>}{u.must_change_password&&<span style={S.tag("#EF4444")}>⚠ PW ändern</span>}</div><div style={{ fontSize:12, color:"#555570" }}>{u.email}</div></div>
          </div>
        ))}
      </div>
...
export default function App() {
  ...
  const loadAll = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const [u, s, p, sl, sc] = await Promise.all([
        db.select("users"), db.select("shoots"), db.select("shoot_participants"), db.select("shotlist"), db.select("schedule")
      ]);
  ...
  return (
    <div style={S.root}>
      ...
      <div style={S.main}>
        {loading ? (
          <div style={{ textAlign:"center", padding:60, color:"#555570" }}>Lade Daten...</div>
        ) : <>
          ...
          {page==="calendar"&&(
            <div>
              ...
              <CalendarView shoots={visibleShoots} user={user} setSelectedShoot={setSelectedShoot} setPage={setPage}/>
            </div>
          )}
          ...
          {page==="shoot-detail"&&selectedShoot&&(
            <ShootDetail shoot={selectedShoot} setShoot={setSelectedShoot} participants={participants} setParticipants={setParticipants} shotlist={shotlist} setShotlist={setShotlist} schedule={schedule} setSchedule={setSchedule} users={users} user={user} setPage={setPage} onDelete={handleDeleteShoot}/>
          )}
        </>}
      </div>
    </div>
