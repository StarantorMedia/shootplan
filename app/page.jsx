...
function ShootDetail({ shoot, setShoot, participants, setParticipants, shotlist, setShotlist, schedule, setSchedule, users, clients, user, setPage, onDelete, userEquipment }) {
  ...
  return (
    <div>
      ...
      {tab === "shotlist" && (
        <ShotlistTab shots={shots} shoot={shoot} user={user} updateShot={updateShot} deleteShot={deleteShot} setShotlist={setShotlist} />
      )}
...
// ============================================================
function ShotlistTab({ shots, shoot, user, updateShot, deleteShot, setShotlist }) {
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
        ...
        {user.is_admin && <button style={S.btn("primary")} onClick={addNewPart}>＋ Teil</button>}
      </div>
      ...
      {totalShots === 0 && (
        <div style={{ ...S.card, textAlign: "center", padding: 48, color: C.textDim }}>
          ...
          {user.is_admin && <button style={{ ...S.btn("primary"), marginTop: 14 }} onClick={addNewPart}>Ersten Teil erstellen</button>}
        </div>
      )}
      ...
      {Object.entries(grouped).map(([partNum, scenes]) => (
        <div key={partNum} style={{ marginBottom: 20 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10, padding: "8px 12px", background: C.surfaceHi, border: `1px solid ${C.borderHi}`, borderLeft: `3px solid ${C.accent}`, borderRadius: 2 }}>
            ...
            <div style={{ flex: 1 }}/>
            {user.is_admin && <button style={{ ...S.btn("ghost"), padding: "3px 10px", fontSize: 10 }} onClick={() => addSceneToPart(parseInt(partNum))}>＋ Szene</button>}
          </div>
          ...
          {Object.entries(scenes).map(([sceneNum, shotList]) => (
            <div key={sceneNum} style={{ marginBottom: 12, marginLeft: 12 }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6, padding: "5px 10px", background: C.surface, border: `1px solid ${C.border}`, borderLeft: `2px solid ${C.purple}`, borderRadius: 2 }}>
                ...
                <div style={{ flex: 1 }}/>
                {user.is_admin && <button style={{ ...S.btn("ghost"), padding: "2px 8px", fontSize: 10 }} onClick={() => addShotToScene(parseInt(partNum), parseInt(sceneNum))}>＋ Shot</button>}
              </div>
                  ...
                  return (
                    <div key={shot.id} style={{ ...S.card, padding: "10px 12px", borderLeft: `2px solid ${stColor}` }}>
                      ...
                      <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
                        ...
                        {user.is_admin && <button style={{ ...S.btn("danger"), padding: "3px 8px", fontSize: 10 }} onClick={()=>deleteShot(shot.id)}>✕</button>}
                      </div>
                      ...
                      {isOpen && (
                        <div style={{ marginTop: 10, paddingTop: 10, borderTop: `1px solid ${C.border}`, display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: 8 }}>
                          <div style={{ gridColumn: "1/-1" }}><label style={S.label}>Beschreibung</label>{user.is_admin ? <textarea style={{ ...S.textarea, minHeight: 52 }} value={shot.description||""} onChange={e=>updateShot(shot.id,"description",e.target.value)}/> : <div style={{ ...S.input, minHeight: 52, color: C.textMid, paddingTop: 8 }}>{shot.description||"—"}</div>}</div>
                          <div><label style={S.label}>Kamera / Optik</label><input style={S.input} value={shot.camera_setting||""} onChange={e=>updateShot(shot.id,"camera_setting",e.target.value)} placeholder="24mm f/2.8" readOnly={!user.is_admin}/></div>
                          <div><label style={S.label}>Dauer (hh:mm)</label><input style={S.input} value={shot.duration||""} onChange={e=>updateShot(shot.id,"duration",e.target.value)} placeholder="00:30" readOnly={!user.is_admin}/></div>
                            ...
                            <div style={{ display: "flex", gap: 10, alignItems: "flex-start" }}>
                              {shot.image_url ? (
                                <div style={{ position: "relative" }}>
                                  ...
                                  {user.is_admin && <button style={{ position: "absolute", top: 2, right: 2, background: "rgba(0,0,0,0.7)", border: "none", color: "white", borderRadius: 2, cursor: "pointer", fontSize: 10, padding: "1px 5px" }} onClick={() => updateShot(shot.id, "image_url", "")}>✕</button>}
                                </div>
                              ) : (
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          ...
          {myNetworks.length === 0 && (
            <div style={{ ...S.card, textAlign: "center", padding: 48, color: C.textDim }}>
              ...
              <div style={{ fontSize: 11, marginBottom: 16 }}>{user.is_admin ? "Erstelle ein Netzwerk und lade Crew ein." : "Trete einem Netzwerk bei oder warte auf eine Einladung."}</div>
              {user.is_admin && <button style={S.btn("primary")} onClick={() => setShowCreateModal(true)}>Netzwerk erstellen</button>}
            </div>
          )}
